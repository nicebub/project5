...
int main(const int argc, const char ** argv){
	...
	Machine::program_t program{};
		...
		program.bp = 1;
		program.acc = 0;
		program.program_mem[1] = m.encode(e_instruction::MOV,e_argument_type::REG, e_argument_type::MEM ); // MOV AL, 66
		program.program_mem[2] = static_cast<uint8_t>(e_register::AL); // AL register
		program.program_mem[3] = 42; // decimal 42 hex 2a
    program.program_mem[4] = m.encode(e_instruction::MOV, e_argument_type::REG, e_argument_type::IND); // MOV
    program.program_mem[5] = static_cast<uint8_t>(e_register::CL); // AL register
    program.program_mem[6] = 61; // decimal 42 hex 2a

    program.program_mem[7] = m.encode(e_instruction::MOV, e_argument_type::REG, e_argument_type::MEM); // MOV DL, 42
		program.program_mem[8] = static_cast<uint8_t>(e_register::DL);
		program.program_mem[9] = 66; // decimal 66 hex 42
		program.program_mem[10] = m.encode(e_instruction::HALT); // HALT with return value of dl register
